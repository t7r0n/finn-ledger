...
COMPANY = 'Finvest'
REPO = 'finn-ledger'
PROJECT_TERMS = ['autopilot', 'wealth', 'manager', 'executes', 'money', 'moves', 'optimizes', 'chatbot']
PROJECT_METRICS = ['autopilot_coverage', 'wealth_risk', 'manager_precision', 'executes_latency']
PROJECT_FAILURES = ['autopilot_drift', 'wealth_gap', 'manager_misroute', 'executes_blindspot']
PROJECT_ARCHETYPES = [{'name': 'autopilot evidence replay', 'trigger': 'autopilot signal changes while wealth context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'wealth boundary probe', 'trigger': 'wealth handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'manager regression harness', 'trigger': 'manager behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'executes operator packet', 'trigger': 'executes output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A deterministic, replayable "decision ledger" for AI wealth managers: every trade Finn proposes is a typed, simulated, wash sale safe MoveProposal with a tax impact diff and a counterfactual the user can accept, reject or amend in one tap.'
...
HERO_TITLE = 'Finvest Evidence Triage Board'
